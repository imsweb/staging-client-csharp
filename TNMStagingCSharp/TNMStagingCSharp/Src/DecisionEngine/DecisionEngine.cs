...
namespace TNMStagingCSharp.Src.DecisionEngine
{
    ...
    public class DecisionEngineClass
    {
        ...
        //========================================================================================================================
        public List<IMapping> getInvolvedMappings(IDefinition definition, Dictionary<String, String> context)
        {
            ...
            {
                foreach (IMapping mapping in definition.getMappings())
        ...
        {
            IDefinition definition = getProvider().getDefinition(definitionId);

            if (definition == null)
                throw new System.InvalidOperationException("Unknown starting table: '" + definitionId + "'");
        ...
        // @return a set of table identifiers
        //========================================================================================================================
        public HashSet<String> getInvolvedTables(IDefinition definition)
        {
            HashSet<String> tables = new HashSet<String>();

            // first, evaluate inputs and outputs
            foreach (String key in definition.getInputMap().Keys)
            {
                IInput input = definition.getInputMap()[key];
                if (input.getTable() != null)
                    getInvolvedTables(getProvider().getTable(input.getTable()), tables);
            }
            foreach (String key in definition.getOutputMap().Keys)
            {
                IOutput output = definition.getOutputMap()[key];
        ...
        //   It will also deal with mapped inputs.
        // @param definition a Definition
        // @return a Set of Strings contianing the unique Definition input keys
        //========================================================================================================================
        public HashSet<String> getInputs(IDefinition definition)
        {
            HashSet<String> inputs = new HashSet<String>();
        ...
        // @return a Set of Strings containing the unique Mapping output keys
        //========================================================================================================================
        public HashSet<String> getOutputs(IDefinition definition)
        {
            HashSet<String> outputs = new HashSet<String>();
        ...
        // @return a Result
        //========================================================================================================================
        public Result process(String definitionId, Dictionary<String, String> context)
        {
            IDefinition start = getProvider().getDefinition(definitionId);

            if (start == null)
                throw new System.InvalidOperationException("Unknown definition: '" + definitionId + "'");
        ...
        // @return a Result
        //========================================================================================================================
        public Result process(IDefinition definition, Dictionary<String, String> context)
        {
            ...
            foreach (String key in definition.getInputMap().Keys)
            {
                ...
                if (value.Length > 0 && input.getTable() != null)
                {
                    ...
                    if (endpoints == null)
                    {
                        ...
                        if ((StagingInputErrorHandler.FAIL == definition.getOnInvalidInput()) || (input.getUsedForStaging()
                                && (StagingInputErrorHandler.FAIL_WHEN_USED_FOR_STAGING == definition.getOnInvalidInput())))
            ...
            foreach (KeyValuePair<String, IOutput> entry in definition.getOutputMap())
            ...
            if (definition.getInitialContext() != null)
                foreach (IKeyValue keyValue in definition.getInitialContext())
            ...
            if (definition.getOutputMap() != null && definition.getOutputMap().Count > 0)
            {
                ...
                foreach (KeyValuePair<String, String> entry in context)
                {
                    if (!definition.getOutputMap().ContainsKey(entry.Key))
                    {
                ...
                while (iter.MoveNext())
                {
                    ...
                    if (definition.getOutputMap().ContainsKey(entry.Key))
                    {
                        output = definition.getOutputMap()[entry.Key];
                    }
